...
$stmt = $pdo->prepare("SELECT albums.id, albums.name, albums.year, albums.sale, albums.vinyl_count, albums.cover, albums.description, vinyl_types.type_name, s.state FROM albums join vinyl_types on albums.vinyl_type_id=vinyl_types.id join states s on s.id = albums.state_id where albums.owner_id = :ow_id");
...
$smarty->display("templates/collection.tpl");
